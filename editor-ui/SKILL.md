...
Five things that only show up once it is built:
...
5. **The panel has to be able to say "the file disagrees with me".** A value of the wrong kind shows the description's default; a default shown as though it were the file's own number is U10's failure exactly. So the reader returns *what to show* and *whether it is really that*, and the section carries a line naming the fields that disagree. It is also the assertion that stops a well-meaning panel from "fixing" the file it cannot read.
...
- `kernel-2d/editor/panels/ComponentFields.tsx` — U47: the generated fields themselves, the four states an entity can be in about a described component, and the two rules that differ from the hand-written controls beside them (buttons over sentinels, spread over replace).
