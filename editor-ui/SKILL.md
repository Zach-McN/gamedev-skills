...
**What this did not become, and the reason is the interesting part.** Drawing a road was expected to need a tile painter — a panel, a grid, a brush. It turned out to need a snap and a press, both genre-neutral, both about a hundred lines, and the tile painter never had to exist. The rule that produced that (`genre-spinup` S1) sends work at the game's own vocabulary rather than at a panel; the corollary this session adds is that **the kernel's own gestures getting better is what tedium is answered with**, and a genre tool is for what is impossible rather than for what is slow. ~~Drag-to-paint was not built for the same reason: clicking is quick enough, and a stroke that stamps is the first step toward a brush nobody has asked for.~~ **Two days later a road was still one click per cell, and the stroke was asked for by name (U49) — and it arrived as the same press held down, not as a brush.** The corollary held: the gesture got better, no tool appeared. _[earned 2026-08-13, amended 2026-08-15]_
...
### U49: A paint stroke is the stamp press held down — one run in the history, one copy per cell, the same thing never twice on a cell, and no stroke without a grid

Repeat-placing (U31) armed a prefab so every press stamps one; a road was still a press per cell. Now a press held and moved is a stroke: one copy in every grid cell the pointer crosses until it is let go. It is not a tool, a mode, a brush or a rectangle — it is the press that already existed, continued — and that is the constraint that shaped everything else. Five decisions:

1. **One stroke is one history entry, made through an open *run*, never through per-cell transactions merged afterwards** (`editor-kernel` D7, amended). The merge machinery is keyed and timed for typing one value: a stroke that pauses to think for longer than the window would split into two Ctrl-Zs, and the click after the stroke — same label, plausibly the same key — would fuse into it. A run is identified by nothing but itself, applies each edit as it lands (so the Outliner and the picture move with the hand) and closes on release or Esc. The store test to write is the one that crosses the merge window mid-run and asserts one undo, and the one that stamps immediately after `end()` and asserts a second step.
2. **The same thing is not put down twice on one cell.** Two mechanisms, deliberately: the stroke remembers the cells it has visited (a hand that wobbles back and forth must not stack), *and* the placing recipe itself declines a cell that already holds an instance of the same prefab at that snapped position — so painting back over yesterday's road, in a fresh stroke, adds nothing. What is skipped is exactly *the same prefab in the same place*; a different prefab still stacks, and a plain sprite of the same texture is the sprite branch of the same rule. **A click is a stroke of one and follows it too**, which changed one thing a click used to do (two clicks on one cell used to stack); said in the human page.
3. **The walk between two pointer samples is a line in cell coordinates, not a re-sample of positions.** A fast sweep sees the pointer a few times across many cells; stamping only where it was seen is a dotted road. Bresenham from the last cell to this one, eight-connected, each cell once — a diagonal is a chain of corner-touching cells, as on graph paper — and it lives in a pure module beside `snap.ts` (`stroke.ts`) that names cells by steps from the grid's offset and reaches them through `snapTo`, so a stroke lands exactly where a press would. It reimplements no snapping.
4. **Without the grid there is no stroke, and the drag says so.** A cell is a grid's idea; with the switch off a press places one where it landed as it always did, and a drag from there changes the caption to *Turn Snap on to paint* rather than doing something nobody asked for (U10: a gesture that declines says why). The sentence belongs to the mode and goes with it.
5. **Esc ends the stroke before it ends the mode, and keeps what was placed.** The stroke is a run in the store and the mode is window state; ending the mode with a run still open would leave the next press writing into a closed gesture. So the panel's `stopStamping` ends the stroke first, and Esc mid-stroke leaves one undoable step behind — the honest answer, since the human watched those tiles land.
...
### U51: The window's arrangement is remembered per project, in the browser, behind a versioned envelope of ours — and never in the project folder

Panel arrangement, Assets view and folder reset on every reload for a week (UG4's second half). Remembering them cost one small store and two subscriptions, and four decisions carry:

1. **In the browser, not in the project.** A layout is a fact about this editor window, not about the game. A `.json` in the project would show in the Assets panel as one of the human's files, would need the AI-content marking every generated file needs, and would follow the game into version control where two people's window shapes fight. So it goes where a game's own remembered facts already go (`runtime/web/story-store.ts`): `localStorage`, one small object per key, `kernel2d:layout:<projectPath>` beside `kernel2d:story:<project>`. Clearing site data forgets both, and the human page says so in one breath.
2. **Keyed on the project's absolute path, with a last-project pointer.** The dock mounts before the service has said which project it is looking at, so a naive restore shows the default and then jumps — which reads as the feature not working. The pointer lets `attach` restore the last project's shape immediately; when the tree arrives naming a different project, that one's record replaces it. Two projects opened in turn each come back to their own.
3. **The library's JSON is opaque inside an envelope of ours.** `{ format: 'kernel2d.layout', version: 1, dock: <dockview's own>, assets: { view, folder } }`, one Zod schema, `safeParse`; anything unusable — an old version, damaged storage, a shape the library throws on in `fromJSON` — is the default, never an error. A saved arrangement is a convenience and refusing to open the editor over one is the wrong size of consequence. Each half writes itself (`writeLayout` merges), so the dock and the Assets panel do not know about each other's save.
4. **What is deliberately not remembered:** the split fraction, the open tree folders, the back/forward trail. Not asked for, and each is a case where "back where you were" is less obviously right than "back at the top". *Reset layout* on the Windows menu is one press to the default *and* forgets the record — the reason a remembered layout can never trap anybody, which UG4 was about.

Three things found building it: **the Assets restore must be a state flag, not a ref**, or the writer effect in the same commit writes the defaults over the record it was about to read; **the default must not be laid out twice** — once at attach and again when the project arrives with nothing saved — because `clear()` + relayout unmounts every panel for a frame, which a test that measures a panel straight after load catches as "not on screen" (so the default is recorded as belonging to no project, and a project arriving with no record adopts it); and **a tab's close mark is only clickable when the tab is in front**, which the browser test learned by aiming at a hidden one. _[earned 2026-08-16]_
...
**Half fixed 2026-08-14: the panel menu exists** — the Windows menu on the status strip (U38) reopens any closed panel, so the dead end is gone. ~~Layout persistence is still the other half: the *arrangement* resets on reload, and that remains the feature saved layouts will be.~~ **Fully fixed 2026-08-16 (U51):** the arrangement is remembered per project and *Reset layout* on the same menu is the way back from any shape. _[amended 2026-08-16]_
...
- `kernel-2d/editor/shell/layout-store.ts` — U51: where the window's arrangement is remembered, its envelope, and why the browser and not the project.
...
- `kernel-2d/editor/shell/stroke.ts` and `usePlacePrefab.ts` — U49: cells named by steps from the grid's offset and reached through the snap, the walk between two samples, and the stroke that stamps through one run and skips what is already there. `place-into-scene.ts` holds `alreadyPlacedAt` and the `into` door a run writes through.
...
- ~~Remembering the Assets view or the folder across a reload.~~ Built 2026-08-16 with saved layouts (U51). Not remembered: the split, the open tree folders, the trail — it is window state on purpose (U34); persisting it is the same feature as saved layouts (UG4) and belongs with it.
...
- ~~Saved layouts (UG4's second half — the panel menu is U38, and the arrangement still resets on reload).~~ Built 2026-08-16 (U51).
