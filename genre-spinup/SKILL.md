...
**Applied 2026-09-02 to "a prefab can hold parts", and the answer was kernel, on the first game — recorded because it looks like a breach and is not.** The platformer's fire bar (a block, an arm that turns, fire on the arm) needed a placeable, tunable *group*. Three facts put the primitive in the kernel rather than in the game folder: the platformer had *already* been splitting groups it could not express into pairs of prefabs (flag pole and flag top, pipe body and pipe top) — the second consumer was inside the first game; the piece lifted is the smallest general one (a list of parts on the prefab format and one merge, `editor-kernel` D25 amended), and nothing about fire, arms or turning went with it; and the tower defense's turret-on-a-base is the obvious next want, which is the hypothetical-second-game test only as a check on the direction, not as the justification. **The hazard itself stayed in the game**: the fire bar is a prefab file, one texture, one spin listed and one read moved onto `worldTransformOf` (`games/platformer-2d` game-content T10, game-code C12) — no kernel component, no new panel. The G5 test was passed the honest way: the noun was written into the genre spec first, at the human's request, before anything was built. _[applied 2026-09-02]_
...
**And the particle phase found the third answer, which neither question had a name for.** Fading particles need opacity; opacity *passes* the "could a system compute it?" test — a system knows perfectly well how faint a dying spark should be — so it is not a seam. But it is not content either: the scenery's baked fade works only because that fade never changes, and no arrangement of entities expresses a value that moves every frame. It was a **field the format was simply missing** (`editor-kernel` D34), next to scale and rotation, and it cost one optional line. So the ladder of questions is three rungs, not two: *is this a fact only the host holds?* (seam) → *can it be expressed as entities and art?* (content) → *is it a property the format never got round to carrying?* (field). Reach for the third only after the first two have been answered no, or every renderer feature becomes a format change. _[earned 2026-08-15, second spin-up's particle phase]_
...
- A way for a game folder to put an authoring surface of its own in front of the human.
  - **Trigger:** the first genre tool that cannot be done with the kernel's existing gestures. It has *not* fired yet, and the near miss is the useful part of this entry: drawing a road was expected to need a tile painter and turned out to be reachable by placing a prefab twenty-six times, because S1's rule sent the work at the game's own vocabulary rather than at a panel. The trigger is a tool that is genuinely impossible this way, not one that is merely tedious — tedium is answered by the kernel's own placement gestures getting better, which is genre-neutral and needs none of this.
    ...
    **The level select made it six on 2026-08-14, and this one looked the least like a scene of anything yet.** "Minimal; a menu" reads as the first genuine UI screen — and a menu turned out to be *a scene*: one numbered-banner prefab per level, placed like any content, each carrying a `portal` component naming the scene it opens; completion is a green check the game hangs on banners whose fact says won. What it flushed out honestly, exactly as the wave list flushed out the keyboard, was the two kernel seams a menu cannot fake: the door (a game asking its host to open another scene) and the story (facts remembered between runs) — `editor-kernel` D30 — plus the export learning to walk doors (`text-formats` T20). The shop row's ware mark and the upgrade ladder rode the same day without touching the parked question at all: merchandise is a marked placed entity, a tier ladder is type data on the prefab, and how far a tower has climbed is run state shown as star-pip entities. Six dodges: the panel instinct has not been right once.

    **The HUD itself made it seven, same day** — the last named candidate for "something with no honest home on the board" (range circles before buying, tier prices before clicking, an on-screen key listing) became *pause-gated entities*: the game's own pause is the spec's look-at-the-board phase, so that is when rings, gold digit textures and an authored legend panel lie over the picture, and unpausing removes them (`games/tower-defense/.claude/skills/game-code` T17). A number the art cannot know in advance is composed from ten digit textures (plus a decimal dot, once rungs raised rates); a circle of any radius is one ring texture scaled; what an upgrade buys is stat-icon-and-number pairs; how many waves remain is the queue's own flag texture with a count — the confiscated queue said out loud. Seven dodges. The candidate list for a real UI layer is now empty, which is itself the finding.

    **The second game brought the case the seventh dodge could not cover — UI alive *during* play, on the screen rather than on the board — and it cost one component, not a layer (2026-08-15).** The platformer's coin counter, banner and hint cannot be pause-gated (a platformer is never paused) and cannot follow the camera (a system does not know the window). What it took was `screen`, a kernel component that pins an entity to a corner of the canvas (`editor-kernel` D32) — a *seam*, by the same test that admitted input and the camera: the fact it needs is one only the host holds. Everything on top of it is the tower defense's own moves again: text is pixel art baked into card textures, a live number is digit textures composed by game code, a banner is an entity that comes and goes. The panel instinct is 0-for-8; what the two games have established between them is that "UI" on this kernel is *entities plus at most one placement rule*, and the second rule (screen-pinned) arrived only when a game genuinely could not express it as the first (world-placed).
