...
**Tested by play mode, and it held: running a level added no third game.** This was the obvious place to boot one — in every other tool "the game" is a separate thing from "the editor" — and it turned out not to be needed. Pressing Play swaps *which half produced the request* and reuses the same game, canvas, camera and texture cache; Stop swaps it back and has nothing to tear down. Two things fall out that a second game would have had to arrange by hand: the running level draws through the frozen editing camera, which is what makes the two pictures comparable at all (`editor-verification` V24), and every texture is already on the GPU, so pressing Play costs no upload. What makes the swap safe is the entity layer matching by id and destroying what is not in the new list — so nothing of the level being edited survives into the one that is running.

The general form, because "play mode needs its own renderer" is a strong intuition and it is wrong here: **a new game is warranted by a new surface, never by a new mode.** _[earned 2026-08-12, Phaser 4.1.0]_
...
3. **The asset version is a constant, and the URL is the path.** The reasoning is `editor-kernel` D26 point 2's, not restated here; the renderer-side consequence is that each texture reaches the GPU once per build.
...
**Amended 2026-08-15 by the first synthesized sound (`editor-kernel` D33): a game's own effects share this manager and nothing else about it.** A cue is a list of notes, not a file, so none of the machinery above applies — no fetch, no `decodeAudio`, no cache key, no `DECODED_ALL`. What is reused is the manager's `context` (an `AudioContext`) and its `destination` (an `AudioNode` already behind the master volume and mute nodes), so an exported page still owns exactly one audio context and the human's volume still governs. Four specifics:

1. **`WebAudioSoundManager.context` and `.destination` are the whole API needed** — checked against the vendored 4.1.0 types. Guard with the same `instanceof` P8 point 2 uses; there is no synthesizing on the HTML5 fallback and no honest fallback to give it.
2. **A cue arriving while `manager.locked` is dropped, not parked** — the opposite of the music's answer in point 4, and the difference is what the sound *is*. A loop has no moment, so parking it on `UNLOCKED` is right; an effect is a moment, and a moment played late is worse than a moment missed.
3. **`context.state !== 'running'` is a second lock worth checking**, because Phaser suspends the context when the game loses focus. Scheduling on a stopped clock makes appointments that all arrive at once when it resumes.
4. **The read-back state is the audio clock, not a flag.** `playing` is `context.currentTime < (the time the last scheduled note ends)` — read off the context, so it becomes `silent` by itself with nothing watching it. Both hosts publish it (`data-play-sound`, `data-game-sound`), and the arithmetic that schedules and answers `endsAt` lives in `runtime/scene/synth.ts` with no Phaser import, so it is tested in Node.
...
**What it reports, and why the shape is not a count.** The answer carries the frames, the columns and rows, and **`uncoveredRight` / `uncoveredBottom` — two lengths, not one total.** A single "leftover pixels" number is the tempting shape and it is useless to the only caller that wants it: the overlay shades the strip down the right-hand side and the strip along the bottom, and it cannot recover two lengths from one area. Ask of any "what was left over" report which side the consumer needs it on.

**And a `single` slice of an image with no pixels is no frames, not one empty frame.** The whole-image rectangle is the right answer for `single` in every other case, so the zero case reads as an oversight rather than a decision — but a zero-area frame handed to a renderer is a sprite with no picture that still claims to have one, and every measurement downstream then includes a rectangle nobody can see. _[earned 2026-08-11, edges added 2026-08-12 from the first parity drill, Phaser 4.1.0]_
...
- `kernel-2d/runtime/scene/scene-view.ts` — the second surface of P2 as amended: one game, told which scene and which textures, reporting what it drew. Also P8: the level's music, started by a run and read back as a state, and the game's own cues played through the same manager.
- `kernel-2d/runtime/scene/synth.ts` — P8's amendment: what a note *is*, scheduled on the audio context's clock, with no Phaser import so the envelope is tested in Node.
