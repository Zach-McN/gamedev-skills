...
**Amended 2026-09-01:** *every* file, as bytes, not the two kinds the editor writes. The editor can also rename any file, and a renamed picture nothing restored is what W26 turned out to be.
...
### V34: Swapping the machinery under a gesture is proved by the old spec passing *unmoved*, and the new reach is proved by building the situation that needs it

The Outliner's row drag was replaced wholesale — the browser's HTML5 drag-and-drop out, the editor's own pointer press-move-release in (`editor-ui` U37's amendment, 2026-09-02) — so that the wheel could reach the page mid-carry. Two halves, and each has a shape worth reusing.

**The evidence that nothing else changed was that `parenting.spec.ts` passed with no edit at all.** Sixteen tests about what a drop *means* — attaches, detaches, does not move the child, one Ctrl-Z, refuses a descendant — none of which name `dataTransfer`, `draggable` or a drag event, because they drive the gesture as `mouse.down` / `move` / `up` and read the outcome from the level and the renderer's report. That is CLAUDE.md's "tests assert behavior, never implementation" collecting: a suite written that way lets the mechanism be swapped and *tells you* the meaning survived. Had one assertion mentioned the browser's drag, the swap would have needed the spec edited, and an edited spec proves nothing. **When you are about to replace the machinery under a feature, look first at whether its spec would have to change; if it would, fix the spec before the machinery.**

**The new capability needed a level the sample project does not have.** "The wheel scrolls while carrying" only means anything when the list is longer than the panel, so the test presses *Add* until `scrollHeight > clientHeight` (`fillUntilItScrolls` in `tests/editor/carry-row.ts`) rather than assuming five rows overflow — the same rule as V21's "a picture waits for everything it is a picture of", applied to a precondition instead of a paint. It then wheels to the bottom, picks up the last row, wheels back to the top with the button still down, and asserts the panel still reports the carried row (`data-carrying`) before dropping onto the row it brought into view. **A test of reach has to build the distance.** And the refusal side wants the same attribute: with a filter on, `data-carrying` is what shows a row *was* picked up when the drop was refused — otherwise "nothing happened" cannot be told apart from "the gesture is off". _[earned 2026-09-02]_
...
**Root cause found 2026-09-01, and it was never load.** The chain, from the sidecar's own log: a `component-fields` test renames `icon-heart.png` through the file menu, and the shared restore (V14) put back only `.meta` and `.json` — so the heart stayed renamed. Every play-mode test later in file order then failed on the missing icon; Playwright starts a fresh worker after a failure; the worker re-evaluated `playwright.config.ts`, whose top level *rebuilt the whole project folder* under the running dev server; and a folder removed and recreated is one Vite's chokidar has stopped watching, so from then on no edit to `src/` was ever hot-replaced — the "expected 1, received 2". Three fixes, each necessary: the restore snapshots every file as bytes (V14 amended); the config builds fixtures only on its first evaluation, with a flag on the environment the workers inherit (W31); and the module announces its own replacement so the test waits on a fact rather than a clock (V33). What stays true in W26 is the policy: a full-run failure that passes alone is not evidence of a regression — but it *is* evidence of test-to-test leakage, and the sidecar log's "removed assets/ (folder)" line was the fingerprint all along.
...
### W31: `playwright.config.ts` runs in every worker, and a worker starts after every failure — a top-level side effect there fires again mid-run

The config built the sample project and the export at module top level, reasoning that Playwright starts its servers before anything else, so both had to exist by the time the file finished loading. True — and the file is *also* imported by each worker process, and Playwright starts a new worker after a failed test. So one flake anywhere rebuilt the project folder underneath the dev server, which killed hot replacement for the rest of the run (W26). **Fix:** do the work on the first evaluation only and mark it done in `process.env`, which workers inherit; later evaluations just name the folders. Rule: anything in a Playwright config's top level must be idempotent or guarded, because "once" is not what the top level means there. _[earned 2026-09-01]_
...
- `kernel-2d/tests/editor/scene-view.ts`, `fields.ts`, `panels.ts`, `select-asset.ts`, `parse-when-whole.ts`, `carry-row.ts` — the browser suite's shared hands: the viewport and Outliner-row locators, the camera-settled poll, opening a level, the middle of the selected outline, typing into a field, fronting a tab, walking the Assets tree, a poll-safe parse, and carrying an Outliner row (pick up, hold over a third of another row, let go — plus the list's scroll position and "add rows until it scrolls", V34). A spec never imports another spec; a helper a third spec wants goes here (`editor-kernel` G3 item 13).
